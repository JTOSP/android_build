...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^jtosp_") ; then
       JTOSP_BUILD=$(echo -n $1 | sed -e 's/^jtosp_//g')
       export BUILD_NUMBER=$((date +%s%N ; echo $JTOSP_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10)
    else
       JTOSP_BUILD=
    fi
    export JTOSP_BUILD
...
function print_lunch_menu()
{
    ...
    echo
    if [ "z${JTOSP_DEVICES_ONLY}" != "z" ]; then
...
function breakfast()
{
    target=$1
    local variant=$2
    JTOSP_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/jtosp/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
...
function eat()
{
    ...
    if (adb shell cat /system/build.prop | grep -q "ro.jtosp.device=$JTOSP_BUILD");
    then
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $JTOSP_BUILD, run away!"
    fi
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $JTOSP_FIXUP_COMMON_OUT ]; then
...
# Add completions
check_bash_version && {
    dirs="sdk/bash_completion vendor/jtosp/bash_completion"
    for dir in $dirs; do
    if [ -d ${dir} ]; then
